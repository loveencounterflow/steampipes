(function() {
  ...
  this._pull = function(...transforms) {
    var _, duct, exhaust_pipeline, has_local_sink, last, local_sink, local_source, mem_source, mem_sources, original_source, send;
    ...
    duct.mem_sources = mem_sources = [
      ...
      results;
        results = [];
        for (_ = i = 0, ref = transforms.length - 1; (0 <= ref ? i < ref : i > ref); _ = 0 <= ref ? ++i : --i) {
          results.push([]);
        }
        return results;
      })())
