(function() {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.remit = this.$ = function(...P) {
    var arity, ref, transform;
    ref = P, [...P] = ref, [transform] = splice.call(P, -1);
    ...
    if (P.length > 0) {
      // transform.sink   ?=  []
      // transform.send   ?=  ( d ) -> ...
      return this.modify(...P, transform);
    }
    return transform;
  };
  ...
  this.$async = function(method) {
    var R, arity, done, resolve;
    if (arguments.length !== 1) {
      /* TAINT incomplete implementation: surround, leapfrog arguments missing */
      throw new Error("µ77644 surround arguments not yet implemented");
    }
    if ((arity = method.length) !== 3) {
      throw new Error(`µ77644 method arity ${arity} not implemented`);
    }
    resolve = null;
    done = function() {
      if (arguments.length !== 0) {
        throw new Error("µ82081 arguments not allowed");
      }
      return resolve();
    };
    R = (d, send) => {
      return new Promise(async(resolve_) => {
        resolve = resolve_;
        return (await method(d, send, done));
      });
    };
  ...
  this._pull = function(...transforms) {
    var _, buckets, drain, duct, exhaust_async_pipeline, exhaust_pipeline, has_local_sink, last, last_transform_idx, local_sink, local_source, original_source, send, source, tf_idxs;
