(function() {
  'use strict';
  var $, $async, $show, $watch, CND, FS, OS, PATH, SP, alert, badge, debug, defer, echo, help, info, isa, jr, log, rpr, test, type_of, types, urge, validate, warn, whisper;
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._as_chunked_buffers = function(text, size) {
    ...
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ...
      await T.perform([text, splitter], matcher, error, () => {
        return new Promise((resolve, reject) => {
          var pipeline, values;
          values = this._as_chunked_buffers(text, 3);
          pipeline = [];
  ...
  this["demo"] = async function(T, done) {
    ...
    await T.perform(probe, matcher, error, () => {
      return new Promise((resolve, reject) => {
        ...
        source = this._as_chunked_buffers(probe, 10);
        splitter = '"],["';
        pipeline = [];
        // pipeline.push SP.read_from_file path, 10
        pipeline.push(source);
